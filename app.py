...
app = Flask(__name__)
app.secret_key = os.environ.get("SECRET_KEY", "bizhisaab2025secret")
...
REAL_CASH_CUTOFF = os.environ.get("REAL_CASH_CUTOFF", "2026-05-29")
SHOW_LOGIN_HINT = os.environ.get("SHOW_LOGIN_HINT", "").lower() in ("1", "true", "yes")
POSTEX_TOKEN = os.environ.get("POSTEX_TOKEN", "ZmU0N2Q4OTU2NTNlNDg2NGFlMDgwM2E2ZTc3ZTEwMjM6ZDgzMDZiMWI2YjU0NDI2M2E1ODY4MTM1MTU1MGUwNDA=")
DIGI_PHONE = os.environ.get("DIGI_PHONE", "923123456789")
DIGI_PASS = os.environ.get("DIGI_PASS", "12345678")
DIGI_BASE = os.environ.get("DIGI_BASE", "https://dev.digidokaan.pk/api/v1/digidokaan")
PX_BASE = os.environ.get("POSTEX_BASE", "https://api.postex.pk/services/integration/api/order")
...
.card{background:#fff;border:1px solid #E2E8F0;border-radius:10px;padding:16px;margin-bottom:14px;box-shadow:0 1px 3px rgba(15,23,42,.06)}
.met{box-shadow:0 1px 2px rgba(15,23,42,.04)}
...
.mob-menu{display:none;background:none;border:1px solid #E2E8F0;border-radius:7px;padding:6px 10px;font-size:16px;cursor:pointer;color:#374151}
@media(max-width:700px){.mob-menu{display:block}.sb{display:block;position:fixed;z-index:999;height:100%;transform:translateX(-100%);transition:.3s}.sb.open{transform:translateX(0)}.main{margin-left:0}.g2{grid-template-columns:1fr}.grid{grid-template-columns:1fr 1fr}}
</style>
"""
...
def init_db():
    ...
    # Safe migrations — ADD columns only; never drop or rewrite existing data
    for col_sql in [
        "ALTER TABLE courier ADD COLUMN IF NOT EXISTS account_name TEXT DEFAULT 'Default'",
        "ALTER TABLE ad_spend ADD COLUMN IF NOT EXISTS period_from TEXT DEFAULT ''",
        "ALTER TABLE ad_spend ADD COLUMN IF NOT EXISTS period_to TEXT DEFAULT ''",
        "ALTER TABLE ad_spend ADD COLUMN IF NOT EXISTS paid_from_account TEXT DEFAULT ''",
        "ALTER TABLE purchases ADD COLUMN IF NOT EXISTS paid_from_account TEXT DEFAULT ''",
        "ALTER TABLE purchases ADD COLUMN IF NOT EXISTS total_paid REAL DEFAULT 0",
        "ALTER TABLE purchases ADD COLUMN IF NOT EXISTS remaining REAL DEFAULT 0",
        "ALTER TABLE expenses ADD COLUMN IF NOT EXISTS paid_from_account TEXT DEFAULT ''",
        "ALTER TABLE loans ADD COLUMN IF NOT EXISTS account TEXT DEFAULT ''",
    ]:
        try:
            cur.execute(col_sql)
        except Exception:
...
<a href="/expense-summary" class="{'on' if page=='expsum' else ''}">📊 Expense Summary</a>
...
<a href="/logout">Logout</a>
    ...
    <div class="main">
      <div class="topbar"><button type="button" class="mob-menu" onclick="document.getElementById('sidebar').classList.toggle('open')" aria-label="Menu">☰</button><h1>{title}</h1><span style="font-size:11px;color:#6B7280">👤 {session.get('naam','')}</span></div>
...
def login():
    ...
    hint = '<div class="hint">Default: admin / admin123</div>' if SHOW_LOGIN_HINT else ""
    return f"""<!DOCTYPE html><html><head><meta charset="UTF-8">
    <meta name="viewport" content="width=device-width,initial-scale=1">
    <title>BizHisaab — Login</title>{CSS}
    <style>body{{display:flex;align-items:center;justify-content:center;background:linear-gradient(135deg,#0F172A 0%,#1e3a5f 100%);margin-left:0;min-height:100vh}}
    .box{{background:#fff;border-radius:14px;padding:36px;width:340px;max-width:95vw;box-shadow:0 20px 50px rgba(0,0,0,.25)}}
...
def dashboard():
    ...
    pu_paid = qry(conn,"SELECT COALESCE(SUM(total_paid),0) as v FROM purchases").fetchone()["v"] or 0     real_cash = float(inv) + float(co_all) + float(ll) - float(pu_paid) - float(ex_all) - float(ad_all) - float(lw)
...
def purchases():
    ...
    summary_cards = ""
    if is_admin():
        summary_cards = f"""<div class="grid" style="margin-bottom:14px">
    ...
    amt_headers = "<th>Per Unit</th><th>Total</th>" if is_admin() else ""
    pur_rows_list = []
    for r in rows:
...
# ── Courier proxy routes (login required) ─────────────────────────────────────
...
SHOPIFY_STORE = os.environ.get("SHOPIFY_STORE", "womencomforts.myshopify.com")
...
def cashflow():
    ...
    # ── CASH OUT ──────────────────────────────────────────────────
    purchases  = float(qry(conn, f"SELECT COALESCE(SUM(total_paid),0) as v FROM purchases WHERE 1=1 {('AND date>=%s AND date<=%s' if date_from and date_to else ('AND date>=%s' if date_from else ('AND date<=%s' if date_to else '')))}",  p2).fetchone()["v"] or 0) {('AND date>=%s AND date<=%s' if date_from and date_to else ('AND date>=%s' if date_from else ('AND date<=%s' if date_to else '')))}",  p2).fetchone()["v"] or 0)
...
def init_partial_payments_db():
    ...
    try:
        ...
        cur.execute("ALTER TABLE purchase_payments ADD COLUMN IF NOT EXISTS paid_from_account TEXT DEFAULT ''")
